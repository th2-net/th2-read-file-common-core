...
internal class TestAbstractFileReaderSustained : AbstractReaderTest() {
        ...
        try {
            ...
            expectThat(argumentCaptor.allValues.flatten())
                ...
                .apply {
                    ...
                    all { get { idBuilder() }.get { sessionAlias }.isEqualTo("log") }
                }
        } finally {
            exec.shutdown()
        }
    ...
    internal fun `stops reading data for stream when it fails the validation`() {
        ...
        doReturn(
            listOf(
                RawMessage.builder().apply { idBuilder().setTimestamp(now) },
                RawMessage.builder().apply { idBuilder().setTimestamp(now.minusSeconds(1)) }
            )
